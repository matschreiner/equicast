...
import torch
from tito.model import device, embedding
...
class SE3Message(device.Module):
    def __init__(self, n_features=64, n_reduced_features=0, length_scale=10.0, attention=False):
        ...
        self.positional_encoder = embedding.PositionalEncoder(
            n_hidden_features, max_length=length_scale
        )
    ...
    def forward(self, batch):
        ...
        if self.n_reduced_features > 0:
            invariant_node_features = self.reducer.reduce_invariant_node_features(
                invariant_node_features
            )
            invariant_edge_features = self.reducer.reduce_invariant_edge_features(
